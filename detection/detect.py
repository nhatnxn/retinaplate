...
session = onnxruntime.InferenceSession('point_net.onnx')
# for n, input in enumerate(session.get_inputs()):
#     print(n, input)
...
class Retina_Detector:
    ...
    def detect(self,img):
        ...
        img_cp = img if isinstance(img, list) else [img]
        feed = dict([(input.name, img_cp[n]) for n, input in enumerate(session.get_inputs())])
        print(np.shape(feed['input.1']))
        loc, conf, landms = session.run(None, feed)
        # print(len(x))
        # x = torch.randn(1, 3, 640, 1080).to(self.opt.device)
        # input_names = ['input.1']
        # # # output_names = ['output.1']
        # dynamic_axes = {'input.1': {0 : 'batch', 2 : 'height', 3: 'width'}}
        # torch.onnx._export(self.net, x,  "point_net.onnx", export_params=True, opset_version=11, input_names=input_names, dynamic_axes=dynamic_axes)  # forward pass
        print('net forward time: {:.4f}'.format(time.time() - tic))

        loc = torch.from_numpy(loc).to(self.opt.device)
        conf = torch.from_numpy(conf).to(self.opt.device)
        landms = torch.from_numpy(landms).to(self.opt.device)
        t1=time.time()
